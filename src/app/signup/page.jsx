...
export default function Login() {
  ...
  function parseFullName(fullName) {
    ...
    if (spaceIndex === -1) {
      ...
      const surname = trimmed.slice(0, spaceIndex);
      const firstName = trimmed.slice(spaceIndex + 1).trim();
  ...
  const handleSignUp = async (e) => {
    ...
    if (!fullName?.trim()) {
      setMessage(
        "Full name is required. Enter your surname first, followed by your given name.",
      );
      setErrorOpen(true);
    ...
    if (password !== confirmPassword) {
      setMessage("Password must be the same as the confirm password.");
      setErrorOpen(true);
      setErrorTrigger((prev) => prev + 1);
      return;
    }
    if (!checked) {
      setMessage(
        "Please read and agree to the Terms and Privacy Policy before continuing.",
      );
      setErrorOpen(true);
      setErrorTrigger((prev) => prev + 1);
      return;
